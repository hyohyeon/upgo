...
@WebServlet(value = "/twoboard/twodetail.action")
public class twoBoardDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//3. 조회수 증가 처리
		twoboard.setReadCount(twoboard.getReadCount() + 1);
		...
		req.setAttribute("twoboard", twoboard);//comments에서도 사용
		req.setAttribute("pageno", currentPage);
		
		//5. detail.jsp로 이동
		RequestDispatcher dispatcher = 
			req.getRequestDispatcher("/WEB-INF/views/twoboard/twodetail.jsp");
