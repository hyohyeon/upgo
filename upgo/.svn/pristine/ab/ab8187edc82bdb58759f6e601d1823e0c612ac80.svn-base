...
<head>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('.map').maphilight();
            <!--initiate maphilight function -->
                                    
            $('.region').on('click',function(event){
            	var departregion = this.getAttribute("title");
            	<!-- set var departregion as selected region's name like AK, AL, FL from attribute named title -->
                $('#departregion').val(departregion);
                <!-- set input(hidden) value as var departregion -->
                $("#ir").submit();
                <!-- form(ir) submit to regionchoiceto.action -->
            });
...
<!-- get session data from loginuser -->
<h1 class="light-blue">Hi, <%= member.getMemberId() %>!! First, Pick Where You Depart!!</h1>
	<img class="map" src="../img/usamapword.jpg" width="2000" height="1400" usemap="#usa">
	<!-- set img att like class, src, width, height, usemap-->
	<map name="usa">
...
<form id="ir" action="/upgo/scheduling/regionchoicefrom.action" method="post">
