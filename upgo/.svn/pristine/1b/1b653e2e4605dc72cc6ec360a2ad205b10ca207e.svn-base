<%@page import="com.upgo.dto.BoardComment"%>
...
<head>
	...
	<link rel="Stylesheet" href="/upgo/styles/input3.css" />
			...
			<tr>
				<td style="text-align:left;margin:5px;border-bottom: solid 1px">
	                ...
	                <div id='commentedit<%= bcomment.getCommentNo() %>' style="display: none">
						...
						[<%= bcomment.getRegDate() %>] 
						<br /><br />
						<form id="commenteditform<%= bcomment.getCommentNo() %>" action="updatecomment.action" method="post">
						<input type="hidden" name="boardno" value="<%= board.getBoardNo() %>" />
						<input type="hidden" name="pageno" value='<%= request.getAttribute("pageno") %>' />
